...
enum class Kalkulationsmethode(val beschreibung: String) {
    STUFEN(
        "Stufenmodell, d.h. die Gesamtmenge wird in eine Stufe eingeordnet und für die gesamte Menge gilt " +
                "der so ermittelte Preis"
    ),
    ZONEN(
    ...
    VERBRAUCH_UEBER_SLP_GRENZE_FUNKTIONSBEZOGEN_WEITERE_BERECHNUNG_ALS_LGK(
        "Bei einem Verbrauch über der SLP-Grenze (letzte Staffelgrenze überschritten) " +
                "erfolgt die Berechnung funktionsbezogen (s.o.) als LGK."
    )
}
